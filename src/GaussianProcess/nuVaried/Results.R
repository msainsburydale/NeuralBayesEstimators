...
model <- file.path("GaussianProcess", "nuVaried")
...
df     <- estimates_path %>% file.path("merged_test.csv") %>% read.csv
...
# Load in estimates + true parameters
df     <- estimates_path %>% file.path("merged_scenarios.csv") %>% read.csv
likelihood_path <- file.path(estimates_path, "merged_likelihood_scenarios.csv")
if (file.exists(likelihood_path)) df <- rbind(df, read.csv(likelihood_path))

# Load realisations from the model
fields_scenarios <- file.path(intermediates_path, "fields.csv") %>% read.csv
